...
- Throughout the repository, the name `ext_loco` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of ext_loco (in files/directories) to your_fancy_extension_name
...
### Train Implicit One Stage RFM
```bash
python scripts/rsl_rl/ios_train.py --task Template-Isaac-EEPose-Rough-Limx-WF-Tron1A-v0 --headless --logger wandb
```
### Play Implicit One Stage RFM
```bash
python scripts/rsl_rl/ios_play.py --task Template-Isaac-EEPose-Rough-Limx-WF-Tron1A-Play-v0 
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/ext_loco/ext_loco/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/ext_loco"
    ]
}
```
