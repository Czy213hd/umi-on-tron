...
def legs_min_separation(
    ...
    """Penalty if the two legs (ankles/feet) get closer than a target planar distance."""
    asset: Articulation = env.scene["robot"]
    # cache leg link ids
    ...
    # positions in world
    pos = asset.data.body_pos_w[:, ids, :]  # (N,2,3)
    # planar distance XY
    diff_xy = pos[:, 0, :2] - pos[:, 1, :2]
...
def feet_lateral_distance_exp(
    ...
    """Reward the lateral distance between the two feet in the base frame."""
    asset: Articulation = env.scene[asset_cfg.name]
    ...
    # Convert foot positions from world frame to robot base frame.
    relative_pos_w = foot_pos_w - base_pos_w.unsqueeze(1)
    base_quat = base_quat_w.unsqueeze(1).expand(-1, foot_pos_w.shape[1], -1)
    foot_pos_b = math_utils.quat_rotate_inverse(base_quat, relative_pos_w)

    # The Y axis is the left-right direction in the base frame.
    lateral_distance = torch.abs(foot_pos_b[:, 0, 1] - foot_pos_b[:, 1, 1])
    error = lateral_distance - target_distance
