...
class MySceneCfg(InteractiveSceneCfg):
    ...
    terrain = TerrainImporterCfg(
        ...
        terrain_type="plane",  # flat ground instead of rough generator
...
class CommandsCfg:
    ...
    EE_pose = mdp.PicklePoseSequenceCommandCfg(
        ...
        file_path="/home/phi5090ii/NYX/umi-on-tron-lab/IsaacLab_RFM/data/tossing.pkl",
        planar_center=True,
...
class RewardsCfg:
    ...
    # Safety
    safety_exp = RewTerm(
        func=mdp.safety_reward_exp, weight=0.5, params={"base_height_target": 0.8, "std": math.sqrt(0.5)}
    )
    ...
    # EE Tracking
    # track_EE_position_exp = RewTerm(func=mdp.track_EE_position_exp, weight=1.0, params={"command_name": "EE_pose", "std": math.sqrt(0.5)})
    # track_EE_orientation_exp = RewTerm(func=mdp.track_EE_orientation_exp, weight=1.0, params={"command_name": "EE_pose", "std": math.sqrt(0.5)})
    # track_EE_pb = RewTerm(func=mdp.track_EE_pb, weight=1.0)
    # track_EE_reference_exp = RewTerm(func=mdp.track_EE_reference_exp, weight=1.0, params={"std": math.sqrt(0.5), "init_value": 0.98})
