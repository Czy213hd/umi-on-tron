...
from ext_loco.assets.limx import LIMX_WF_TRON1A_ARM
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    # robots
    robot: ArticulationCfg = LIMX_WF_TRON1A_ARM.replace(prim_path="{ENV_REGEX_NS}/Robot") # type: ignore
...
class CommandsCfg:
    ...
    EE_pose = mdp.UniformWorldPoseCommandCfg(
        asset_name="robot",
        body_name="link6",
        # resampling_time_range=(float("-inf"), float("inf")),  # not used
        ...
        ranges=mdp.UniformPoseCommandCfg.Ranges(
            ...
            pos_z=(0.7, 1.6),  # min max [m]
            roll=(-3.2, 3.2),  # min max [rad]
            pitch=(-3.2, 3.2),  # min max [rad]
            yaw=(-3.2, 3.2),  # min max [rad]),
        ),
        # sensor_cfg=SceneEntityCfg("contact_forces", body_names=BODY_NAME_REGEX),
        # threshold=CONTACT_THRESHOLD,
        se3_decrease_vel_range=(0.5, 1.4),
        # resampling_time_scale=(0.5, 2.5),
        debug_vis=True,
    )
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(asset_name="robot", joint_names=["abad_[RL]_Joint","hip_[RL]_Joint","knee_[RL]_Joint","J.*"], 
...
class ObservationsCfg:
    ...
    class NextObsCfg(ObsGroup):
        ...
        foot_position_b = ObsTerm(
            ...
            params={"asset_cfg": SceneEntityCfg("robot", body_names="wheel_[RL]_Link")},
        )  # 197
    ...
    class CriticCfg(PolicyCfg):
        ...
        rigid_body_materials = ObsTerm(
            ...
            params={"asset_cfg": SceneEntityCfg(name="robot", body_names=["wheel_.+", "base_Link", "link6"])},
        ) 
...
class RewardsCfg:
    ...
    track_EE_position_exp = RewTerm(
        func=mdp.track_EE_position_exp,
        weight=2.0,
        params={
    ...
    track_EE_orientation_exp = RewTerm(
        func=mdp.track_EE_orientation_exp,
        weight=3.0,
        params={
            "command_name": "EE_pose",
            "std": math.sqrt(0.5),
    ...
    # track_EE_penalty_cb = RewTerm(func=mdp.tracking_EE_cb_penalty_l1, weight=-1.0)
    track_EE_pb = RewTerm(func=mdp.track_EE_pb, weight=15.0)

    track_EE_reference_exp = RewTerm(
        func=mdp.track_EE_reference_exp,
        weight=1.5,
    ...
    dof_vel_arm_l2 = RewTerm(
        func=mdp.joint_vel_l2, weight=-0.005, params={"asset_cfg": SceneEntityCfg("robot", joint_names="J.*")}
    )
...
class CurriculumCfg:
    ...
    # terrain_levels = CurrTerm(func=mdp.terrain_levels_random)
