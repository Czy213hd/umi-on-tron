 ...
 */
 ...
 namespace pointfoot_gazebo
 {
                           ...
                           const urdf::Model *urdf_model, std::vector<transmission_interface::TransmissionInfo> transmissions)
     {
         ...
         // first frame of cmdbuffer is read before callback receive
         airbot_msgs::ArmCmd init_arm_cmd;
         init_arm_cmd.q.resize(6, 0);
         init_arm_cmd.v.resize(6, 0); // in limxsdk v is replaced by dq
         init_arm_cmd.tau.resize(6, 0);
         init_arm_cmd.kp.resize(6, 0);
         init_arm_cmd.kd.resize(6, 4.0);
         armCmdBuffer_.writeFromNonRT(init_arm_cmd);
         ...
         armStatePub_ = model_nh.advertise<airbot_msgs::ArmState>("/ArmState", false, 10);
 
         armCmdSub_ = model_nh.subscribe<airbot_msgs::ArmCmd>("/ArmCmd", 10, &PointFootHWSim::armCmdCallback, this);
         return true;
     }
     ...
     void PointFootHWSim::readSim(ros::Time time, ros::Duration period)
     {
         ...
         if (joint_names_.size() == motor_number + 6) // arm
         {
             limxsdk::RobotState state;
 
             state.stamp = ros::Time::now().toNSec();
             state.q.resize(motor_number);
             state.dq.resize(motor_number);
             state.tau.resize(motor_number);
 
             // Fill in joint position, velocity, and effort values
             for (int i = 0; i < motor_number; i++) // order is L->R joint, and J1->J6
             {
             ...
             // arm command
             airbot_msgs::ArmState arm_state;
             arm_state.header.stamp = ros::Time::now();
             arm_state.q.resize(6);
             arm_state.dq.resize(6);
             arm_state.tau.resize(6);
 
             // Fill in joint position, velocity, and effort values
             for (int i = 0; i < 6; i++)
             {
                 // int index = parseJointIndex(joint_names_[i + motor_number]);
                 int index = int(joint_names_[i + motor_number].back() - '1');
                 if (index != -1) 
                 {
                     ros::Time now = ros::Time::now();
                     arm_state.header.stamp = now;
                     arm_state.q[index] = joint_position_[i + motor_number];
                     arm_state.dq[index] = joint_velocity_[i + motor_number];
                     arm_state.tau[index] = joint_effort_[i + motor_number];
                 }
             }
             armStatePub_.publish(arm_state);
     ...
     void PointFootHWSim::writeSim(ros::Time time, ros::Duration period)
     {
         ...
         auto arm_cmd = *armCmdBuffer_.readFromRT();
 
         // Update joint data with commands from robot command buffer
         for (auto & joint : hybridJointDatas_) // size is 14
         {
             ...
             {
                 int arm_index = int(joint.joint_.getName().back() - '1');
                 joint.posDes_ = arm_cmd.q[arm_index];
                 joint.velDes_ = arm_cmd.v[arm_index];
                 joint.ff_ = arm_cmd.tau[arm_index];
                 joint.kp_ = arm_cmd.kp[arm_index];
                 joint.kd_ = arm_cmd.kd[arm_index];
                // // INSERT_YOUR_CODE
                // ROS_INFO_STREAM("Arm Joint [" << joint.joint_.getName() << "] "
                //                 << "posDes: " << joint.posDes_ << ", "
                //                 << "velDes: " << joint.velDes_ << ", "
                //                 << "ff: " << joint.ff_ << ", "
                //                 << "kp: " << joint.kp_ << ", "
                //                 << "kd: " << joint.kd_);
             }
     ...
     void PointFootHWSim::armCmdCallback(const airbot_msgs::ArmCmdConstPtr& msg) {
         armCmdBuffer_.writeFromNonRT(*msg);
     }
 } // namespace pointfoot_gazebo
