 ...
 */
 ...
 bool PointfootHW::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh) {
   ...
   if(is_sim_ == 0)
   {
     // is_sim_ = 0;
#ifdef USE_ARX5_SDK
    ROS_INFO("Real World Deploy , ARX5 SDK Init!");

    std::string arx_model = "L5";
    std::string can_interface = "can0";
    robot_hw_nh.param<std::string>("/PointfootCfg/arx5/model", arx_model, arx_model);
    robot_hw_nh.param<std::string>("/PointfootCfg/arx5/can_interface", can_interface, can_interface);
    ROS_INFO_STREAM("Initializing ARX5 SocketCAN driver (no solver/no soem). model=" << arx_model
                                                                                     << " can=" << can_interface);

    Arx5SocketCanArm::Config cfg;
    cfg.model = arx_model;
    cfg.can_interface = can_interface;
    robot_hw_nh.param<double>("/PointfootCfg/arx5/gripper_open_pos", arx5_gripper_pos_open_, arx5_gripper_pos_open_);
    robot_hw_nh.param<double>("/PointfootCfg/arx5/gripper_close_pos", arx5_gripper_pos_close_, arx5_gripper_pos_close_);
    robot_hw_nh.param<double>("/PointfootCfg/arx5/gripper_width", cfg.gripper_width, cfg.gripper_width);
    robot_hw_nh.param<double>("/PointfootCfg/arx5/gripper_open_readout", cfg.gripper_open_readout, cfg.gripper_open_readout);
    robot_hw_nh.param<double>("/PointfootCfg/arx5/gripper_kp", cfg.gripper_kp, cfg.gripper_kp);
    robot_hw_nh.param<double>("/PointfootCfg/arx5/gripper_kd", cfg.gripper_kd, cfg.gripper_kd);
    bool enable_motors = true;
    robot_hw_nh.param<bool>("/PointfootCfg/arx5/enable_motors_on_init", enable_motors, enable_motors);
    cfg.enable_motors_on_init = enable_motors;

    arx5_arm_hw_ = std::make_unique<Arx5SocketCanArm>(cfg);
    if (!arx5_arm_hw_ || !arx5_arm_hw_->ok()) {
      ROS_ERROR("Failed to initialize ARX5 SocketCAN driver.");
      return false;
    }
 ...
 void PointfootHW::read(const ros::Time& /*time*/, const ros::Duration& /*period*/) {
   ...
   if(is_sim_)
   {
     ...
     //Real robot
#ifdef USE_ARX5_SDK
    Arx5SocketCanArm::State st;
    const bool ok = (arx5_arm_hw_ != nullptr) && arx5_arm_hw_->read(st);
    if (!ok || !st.valid) {
      // Keep last jointData_ values if read fails (avoid injecting zeros into controller).
      ROS_WARN_THROTTLE(1.0, "ARX5 read() failed; keeping previous arm joint states.");
    } else {
      for (int i = 0; i < 6; ++i) {
        jointData_[i + joint_num_].pos_ = st.q[static_cast<size_t>(i)];
        jointData_[i + joint_num_].vel_ = st.dq[static_cast<size_t>(i)];
        jointData_[i + joint_num_].tau_ = st.tau[static_cast<size_t>(i)];
      }
    }
#else
 ...
 void PointfootHW::write(const ros::Time& /*time*/, const ros::Duration& /*period*/) {
   ...
   if(is_sim_)
   {
     ...
     {
#ifdef USE_ARX5_SDK
      // Build ARX5 command from jointData_ (pos/vel/ff) and gains (kp/kd).
      // We send CAN commands at robot_hw loop rate (default 500Hz).
      // Policy updates posDes_ at ~50Hz (decimation), so we simply resend latest targets.
      std::array<double, 6> q_des{};
      std::array<double, 6> dq_des{};
      std::array<double, 6> kp{};
      std::array<double, 6> kd{};
      std::array<double, 6> tau_ff{};

      // Ramp-in similar to existing Airbot logic (highDampingCnt_ updated at 100Hz below).
      const double ramp = std::min((highDampingCnt_ + 50) / 180.0, 1.0);
      for (int i = 0; i < 6; ++i) {
        q_des[static_cast<size_t>(i)] = static_cast<double>(jointData_[i + joint_num_].posDes_) * ramp;
        dq_des[static_cast<size_t>(i)] = static_cast<double>(jointData_[i + joint_num_].velDes_);
        kp[static_cast<size_t>(i)] = static_cast<double>(jointData_[i + joint_num_].kp_);
        kd[static_cast<size_t>(i)] = static_cast<double>(jointData_[i + joint_num_].kd_);
        tau_ff[static_cast<size_t>(i)] = static_cast<double>(jointData_[i + joint_num_].tau_ff_);
      }

      // Match the previous "high damping" behavior in early phase.
      const double kd_factor = highDampingCnt_ < 200 ? 3.0 : 1.0;
      for (int i = 0; i < 6; ++i) {
        kd[static_cast<size_t>(i)] *= kd_factor;
      }

      // Toggle gripper at 100Hz (same as previous arm_pub_flag cadence).
      if (arm_pub_flag) {
        ...
        if (print_info) {
          ROS_INFO_STREAM("ARX5 gripper target pos (m): "
                          << (maintain_grasp_ ? arx5_gripper_pos_open_ : arx5_gripper_pos_close_));
        }
        highDampingCnt_++;
      }

      const double gripper_pos_target = maintain_grasp_ ? arx5_gripper_pos_open_ : arx5_gripper_pos_close_;

      const bool send_ok = (arx5_arm_hw_ != nullptr) &&
                           arx5_arm_hw_->write(q_des, dq_des, kp, kd, tau_ff, gripper_pos_target);
      if (!send_ok) {
        ROS_WARN_THROTTLE(1.0, "Failed to send ARX5 SocketCAN arm command.");
      }
#else
       for (int i = 0; i < 6; ++i)
       {
 ...
 {
#ifdef USE_ARX5_SDK
  // ARX5 gripper is driven by cmd.gripper_pos inside write() (Arx5JointController).
  (void)to_grasp;
  (void)print_info;
  return;
#else
